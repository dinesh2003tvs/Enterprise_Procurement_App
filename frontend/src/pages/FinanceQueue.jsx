...
import { DollarSign, Check, X, ShieldAlert, Sparkles } from 'lucide-react';
...
export const FinanceQueue = () => {
  ...
  // Simulated Department Budgets (Phase 2 §7 & §31)
  ...
  const openModal = (req, action) => {
    ...
    setComment(action === 'APPROVE' ? 'Departmental budget validated and reserved' : '');
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-6 pb-4 border-b border-slate-200 flex items-center justify-between">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h1 className="text-2xl font-bold text-slate-900">Finance Approval Queue</h1>
          </div>
          <p className="text-xs text-slate-500 mt-1">
            Validate departmental budgets and approve manager-vetted requests (Phase 2 §31)
          </p>
        </div>
      ...
      {loading ? (
        <LoadingState message="Loading finance review queue..." />
      ) : error ? (
        <ErrorState title="Queue Error" message={error} onRetry={fetchPending} />
      ) : requests.length === 0 ? (
        <EmptyState
          title="Finance Queue Clear"
          message="No purchase requests are currently awaiting budget validation."
        />
      ) : (
            ...
            <table className="w-full text-left text-xs text-slate-600">
              <thead className="bg-slate-50 text-slate-700 font-semibold uppercase tracking-wider text-[10px] border-b border-slate-200">
                <tr>
                  ...
                  <th className="py-3 px-4">Dept & Requester</th>
                  <th className="py-3 px-4">Item & Category</th>
                  <th className="py-3 px-4">Purchase Amount</th>
                  <th className="py-3 px-4">Available Budget</th>
                  <th className="py-3 px-4">Remaining After</th>
                  ...
                  return (
                    <tr key={r.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="py-3.5 px-4">
                        ...
                        <div className="text-[10px] text-slate-400">{r.employeeName}</div>
                      </td>
                      <td className="py-3.5 px-4">
                      ...
                      <td className="py-3.5 px-4 font-semibold">
                        <span className={isSufficient ? 'text-slate-800' : 'text-rose-600 font-bold'}>
      ...
      {/* Finance Decision Modal */}
      {selectedReq && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-xl border border-slate-200">
            <h3 className="text-lg font-bold text-slate-900 mb-2">
              {modalAction === 'APPROVE' ? 'Approve Financial Budget' : 'Reject Financial Request'}
            </h3>
            <p className="text-xs text-slate-500 mb-4">
              Validate and allocate departmental budget for <strong className="font-mono text-slate-800">{selectedReq.id}</strong>
            </p>

            <div className="p-4 rounded-xl bg-emerald-50/60 border border-emerald-200 text-xs mb-4 space-y-1.5 text-emerald-900">
              <div className="flex justify-between">
                <span>Department Budget Pool:</span>
                <span className="font-bold">₹{departmentBudgets[selectedReq.department]?.toLocaleString('en-IN') || '3,00,000'}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between pt-1 border-t border-emerald-200 text-emerald-950 font-bold">
                <span>Remaining Pool After Approval:</span>
            ...
            <div className="mb-4">
              <label className="block text-xs font-semibold text-slate-700 mb-1">
                Finance Comments {modalAction === 'REJECT' && <span className="text-rose-500">*</span>}
              </label>
              <textarea
                rows="3"
                value={comment}
                onChange={(e) => setComment(e.target.value)}
                placeholder="Finance justification remarks..."
            ...
            <div className="flex items-center justify-end space-x-3">
              ...
              >
                {actionLoading ? 'Processing...' : modalAction === 'APPROVE' ? 'Confirm Allocation' : 'Confirm Rejection'}
              </button>
            </div>
          </div>
        </div>
